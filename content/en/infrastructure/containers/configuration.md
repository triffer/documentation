...
-   **Field**: Groups object array nested fields under each unique value of the selected field.
-   **No field**: Flattens object array nested fields into arrays, merging all field values.
...
**Result:** Returns CR1 and CR2. Each filter is evaluated separately. At least one `status.condition` object in each custom resource matches each filter, even if it's not the same object that matches both filters.
...
**Result:** Returns only CR2. The query filters for CRs where `status.condition` objects of type `HorizontalAbleToScale` have `status:"False"`. Both the `type` and `status` filters must match the same object.
